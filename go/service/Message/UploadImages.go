...
// @Description upload images type
// @Tags message
// @Accept multipart/form-data
// @Produce json
// @Param Authorization header string true "Authorization token"
// @Param file formData file true "images to upload"
// @Success 200 {object} map[string]interface{} "id: User ID, message: Upload status message, path: URL to the uploaded video"
// @Failure 400 {object} map[string]string "error: Can not receive the image"
...
func UploadImages(c *gin.Context) {
	file, err := c.FormFile("image") // 这里的 "file" 是前端通过表单上传文件时的字段名
	if err != nil {
