...
// @Param type formData string true "Type of the file"
// @Param file formData file true "File to upload"
// @Success 200 {object} map[string]interface{} "id: User ID, message: Upload status message, path: URL to the uploaded file"
// @Failure 400 {object} map[string]string "error: Can not receive the file"
...
func UploadFiles(c *gin.Context) {
	...
	file, err := c.FormFile("file") // 这里的 "file" 是前端通过表单上传文件时的字段名
	if err != nil {
	...
	*/
	...
	// 构建保存路径
	savePath := "./Files/Files/" + newFileName
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"url":      "http://114.116.204.137/ChatSpace" + savePath,
	})
}
